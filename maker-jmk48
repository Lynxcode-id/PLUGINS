...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        const apiUrl = `https://restapi.jagoanproject.web.id/api/maker/jmk48?image=${encodeURIComponent(imageUrl)}`
        ...
        let caption = `┌˚₊ ๑│ ᴊ ᴍ ᴋ 𝟺 𝟾  ᴍ ᴀ ᴋ ᴇ ʀ │๑˚₊ 🎤\n` +
                      `┇ \n` +
                      `│ ✅ *Sukses membuat kartu JMK48!*\n` +
                      `┇ \n` +
                      `└˚₊ ๑ ────────────── ๑˚₊\n` +
                      `> © ERINE-AI`
        ...
        console.error('[JMK48 ERROR]', error)
        await m.react('❌')
