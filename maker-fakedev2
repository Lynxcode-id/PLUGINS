...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        ...
        const apiUrl = `https://restapi.jagoanproject.web.id/api/maker/fakedev2?url=${encodeURIComponent(imageUrl)}&text=${encodeURIComponent(text)}`
        ...
        let caption = `┌˚₊ ๑│ ғ ᴀ ᴋ ᴇ  ᴅ ᴇ ᴠ  2 │๑˚₊ 💻\n` +
                      `┇ \n` +
                      `│ 📝 *Text:* ${text}\n` +
                      `┇ \n` +
                      `└˚₊ ๑ ────────────── ๑˚₊\n` +
        ...
        console.error('[FAKEDEV2 ERROR]', error)
        await m.react('❌')
...
handler.help = ['fakedev2 <teks>']
handler.tags = ['maker']
handler.command = /^(fakedev2|devfake2)$/i
handler.limit = true
